...
Deno.serve(async (req) => {
  try {
    ...
    // Seed words for the specified date range
    await seedWordsForDateRange(startDate, endDate);
    
    return new Response(
      JSON.stringify({ 
        success: true,
        message: `Successfully seeded words from ${startDate} to ${endDate}` 
      }),
    ...
    return new Response(
      JSON.stringify({ 
        success: false,
        error: error.message || 'Failed to seed words'
      }),
