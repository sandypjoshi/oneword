...
import { Canvas, Group, Vertices } from '@shopify/react-native-skia';
...
export const MeshGradient: React.FC<MeshGradientProps> = ({
  ...
  const createMeshPoints = useCallback((): MeshData => {
    ...
    const centerRadius =
      centerRadiusRange[0] +
      rng() * (centerRadiusRange[1] - centerRadiusRange[0]);
    ...
    for (let y = 0; y < rows; y++) {
      for (let x = 0; x < cols; x++) {
        ...
        for (let i = 0; i < controlPoints.length; i++) {
          ...
          const flowMix1 =
            sin(flowValue1 * PI * 1.5 + noise.angle * 0.4) * 0.5 + 0.5;
          const flowMix2 =
            sin(flowValue2 * PI * 1.5 - noise.angle * 0.5) * 0.5 + 0.5;
          const flowMix3 =
            sin(flowValue3 * PI * 1.5 + noise.angle * 0.3) * 0.5 + 0.5;
        ...
        const r = min(
          255,
          max(0, Math.round(Math.pow(blendedColor.r, 1 / 2.2) * 255))
        )
          .toString(16)
          .padStart(2, '0');
        const g = min(
          255,
          max(0, Math.round(Math.pow(blendedColor.g, 1 / 2.2) * 255))
        )
          .toString(16)
          .padStart(2, '0');
        const b = min(
          255,
          max(0, Math.round(Math.pow(blendedColor.b, 1 / 2.2) * 255))
        )
          .toString(16)
          .padStart(2, '0');
