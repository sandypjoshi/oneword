...
const Text: React.FC<TextProps> = ({
  ...
  const composedStyle = useMemo(() => {
    ...
    const finalWeightProp: TextStyle['fontWeight'] =
      flatStyle.fontWeight || weight || variantStyle.fontWeight;
    const resolvedWeight = getResolvedWeight(finalWeightProp);
    // Prioritize italic prop/style over variant's default
    const finalItalic =
      flatStyle.fontStyle === 'italic' ||
      italic ||
    ...
    if (fonts) {
      if (baseFamilyType === 'serif') {
        ...
        if (variant?.toLowerCase().includes('display')) {
          selectedFontFamily = finalItalic
            ? fonts.serifDisplayItalic
            : fonts.serifDisplay;
        } else {
        ...
        if (resolvedWeight === '700') {
          selectedFontFamily = finalItalic
            ? fonts.systemBoldItalic
            : fonts.systemBold;
        } else if (resolvedWeight === '500') {
          selectedFontFamily = finalItalic
            ? fonts.systemMediumItalic
            : fonts.systemMedium;
        } else {
          // 400
      ...
      if (
        selectedFontFamily &&
        Object.values(fonts).includes(selectedFontFamily)
      ) {
    ...
    const combinedStyle: TextStyle = {
      ...
      fontWeight: hasSpecificFamily
        ? undefined
        : flatStyle.fontWeight || variantStyle.fontWeight,
      fontStyle: hasSpecificFamily
        ? undefined
        : finalItalic
          ? 'italic'
          : 'normal',
  ...
  const finalStyle = Object.entries(composedStyle).reduce(
    (acc, [key, value]) => {
