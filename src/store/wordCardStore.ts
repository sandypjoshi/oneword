    ...
    persist(
      (set, get) => ({
        ...
        setCardFace: (wordId, face) =>
          set(state => {
            console.log(
              `[wordCardStore.setCardFace] Setting word ${wordId} to face: ${face}`
            );
        ...
        selectOption: (wordId, option, isCorrect) =>
          set(state => {
            console.log(
              `[wordCardStore.selectOption] Word: ${wordId}, Option: ${option}, Correct: ${isCorrect}`
            );
            // Ensure word state exists or create it
            if (!state.words[wordId]) {
              state.words[wordId] = createDefaultWordState();
            ...
            wordState.optionStates[option] = isCorrect
              ? 'correct'
              : 'incorrect';
        ...
        clearSelection: wordId =>
          set(state => {
            console.log(
              `[wordCardStore.clearSelection] Clearing selection for word: ${wordId}`
            );
            ...
            if (state.words[wordId]?.selectedOption !== undefined) {
              ...
              console.warn(
                `[wordCardStore.clearSelection] Word ${wordId} or its selection not found.`
              );
            }
          }),
        ...
        markWordRevealed: (wordId, attempts) =>
          set(state => {
            console.log(
              `[wordCardStore.markWordRevealed] Marking word ${wordId} as revealed`
            );
            // Ensure word state exists or create it
            if (!state.words[wordId]) {
              state.words[wordId] = createDefaultWordState();
            ...
            if (wordState.isRevealed) {
              ...
              if (attempts !== undefined && wordState.attempts !== attempts) {
                console.log(
                  `[wordCardStore.markWordRevealed] Word ${wordId} already revealed, updating attempts to ${attempts}`
                );
        ...
        resetCardState: wordId =>
          set(state => {
            console.log(
              `[wordCardStore.resetCardState] Resetting state for word: ${wordId}`
            );
            ...
            if (state.words[wordId]) {
              ...
              console.warn(
                `[wordCardStore.resetCardState] Word ${wordId} not found. No changes needed.`
              );
            }
          }),
        ...
        _dangerouslyResetAllState: () =>
          set(state => {
            console.warn(
              '[wordCardStore._dangerouslyResetAllState] Resetting all card state'
            );
        ...
        speakWord: async (wordId, word) => {
          ...
          try {
            await speak(word);
            console.log(
              `[wordCardStore.speakWord] Finished speaking wordId: ${wordId}`
            );
            // TODO: Consider adding state update for successful completion if needed by UI
          } catch (error) {
            console.error(
              `[wordCardStore.speakWord] Error speaking wordId ${wordId}:`,
              error
            );
            ...
            set(state => {
              state.speakingWordIds = state.speakingWordIds.filter(
                (id: string) => id !== wordId
              );
            });
          }
      ...
      }),
      {
        ...
        onRehydrateStorage: () => state => {
          if (state) {
            ...
            console.log(
              `[wordCardStore] Successfully rehydrated state: ${wordCount} words found.`
            );
            // Ensure transient state (speakingWordIds) is initialized on rehydrate
            state.speakingWordIds = [];
          } else {
            console.warn(
              '[wordCardStore] Failed to rehydrate state or storage is empty'
            );
          }
        },
      }
