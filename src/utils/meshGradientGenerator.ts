 ...
 */
export function getOrGenerateMesh(
  wordId: string,
...
export function generateMeshGradient(options: MeshGradientOptions): MeshData {
  ...
  const centerOffsetX =
    params.controlPoints.centerOffset.x.min +
    random() * params.controlPoints.centerOffset.x.range;
  const centerOffsetY =
    params.controlPoints.centerOffset.y.min +
  ...
  const numPoints =
    params.controlPoints.count.base +
    Math.floor(random() * params.controlPoints.count.random);
  const baseRadius =
    params.controlPoints.radius.base +
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    const influence =
      params.controlPoints.influence.min +
  ...
  const centerRadiusRange = [
    params.centerPoint.radius.min,
    params.centerPoint.radius.max,
  ];
  const centerRadius =
    centerRadiusRange[0] +
 ...
 */
function generateSimplifiedMesh(
  width: number,
  height: number,
  isDarkMode: boolean,
  customColors?: string[]
