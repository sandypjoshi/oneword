-- Update the existing word_distractors table with additional columns for enhanced functionality
ALTER TABLE word_distractors
    ADD COLUMN IF NOT EXISTS distractor_type VARCHAR(50), -- e.g., 'semantic', 'phonetic', 'taxonomic', 'antonym'
    ADD COLUMN IF NOT EXISTS similarity_score FLOAT,
    ADD COLUMN IF NOT EXISTS creation_method VARCHAR(50), -- e.g., 'wordnet', 'datamuse', 'manual', 'learner_error'
...
CREATE INDEX IF NOT EXISTS idx_word_distractors_quality ON word_distractors(word_id, quality_score);
