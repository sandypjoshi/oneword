...
// Syllable will be loaded dynamically
let syllable;
...
/**
 * Load the syllable module dynamically
 */
async function loadSyllableModule() {
  if (!syllable) {
    try {
      const module = await import('syllable');
      syllable = module.syllable;
      console.log('Syllable module loaded successfully');
    } catch (error) {
      console.error('Error loading syllable module:', error);
      syllable = word => {
        // Simple fallback syllable counter
 ...
 */
async function testSyllableCounting() {
  console.log('\n=== TESTING SYLLABLE COUNTING ===');
  
  // Ensure syllable module is loaded
  await loadSyllableModule();
...
async function runTests() {
  ...
  // Load the syllable module
  await loadSyllableModule();
  
  // Test syllable counting
  await testSyllableCounting();
